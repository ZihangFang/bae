...
from .linearizer import ComponentLinearizer
...
class LM(ppLM):
    def __init__(
        ...
        *args,
        ...
        linearizer: ComponentLinearizer | None = None,
        **kwargs,
    ):
        self.matrix_free_normal = matrix_free_normal or linearizer is not None
    ...
    def step(self, input, target=None, weight=None):
        for pg in self.param_groups:
            ...
            if self.linearizer is not None:
                ...
                linearization = self.linearizer.linearize(
                    self.model,
                    input,
                    target,
                    active_params,
            ...
            if self.linearizer is not None:
                ...
                if self.matrix_free_normal:
                    diag = NormalMatVec._compute_diag(J).clamp(
            ...
            while self.last <= self.loss:
                ...
                try:
                    if self.linearizer is not None:
                        preconditioner = (
                            linearization.make_preconditioner()
                        )
                ...
                self.loss = (
                    self.linearizer.loss(self.model, input, target)
                    if self.linearizer is not None
                    else self.model.loss(input, target)
                )
